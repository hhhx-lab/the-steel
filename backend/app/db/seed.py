...
DEFAULT_PLAN_ID = "plan_beginner_day_1"
DEFAULT_SESSION_ID = "session_local_001"
...
PLAN_EXERCISE_SEED = [
    ("ex_treadmill_warmup", 1, "5 分钟", "current"),
    ("ex_lat_pulldown", 3, "10", "pending"),
    ("ex_chest_press", 3, "10", "pending"),
    ("ex_leg_press", 3, "10", "pending"),
    ("ex_plank", 2, "20 秒", "pending"),
]
...
def seed_database(db: Session) -> None:
    ...
    for equipment in EQUIPMENT_SEED:
        db.merge(Equipment(**equipment))
    for exercise in EXERCISE_SEED:
        db.merge(Exercise(**exercise))
    db.merge(
        WorkoutPlan(
    ...
    for position, (exercise_id, sets, reps, status) in enumerate(PLAN_EXERCISE_SEED, start=1):
        existing = db.scalar(
            select(WorkoutPlanExercise).where(
                WorkoutPlanExercise.plan_id == DEFAULT_PLAN_ID,
                WorkoutPlanExercise.exercise_id == exercise_id,
            )
        )
        if existing is None:
            existing = WorkoutPlanExercise(plan_id=DEFAULT_PLAN_ID, exercise_id=exercise_id)
            db.add(existing)
        existing.sets = sets
        existing.reps = reps
        existing.weight_strategy = "trial_based"
